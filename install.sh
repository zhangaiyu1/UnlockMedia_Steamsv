...
v2ray_conf_hk="${v2ray_conf_dir}/config_hk.json"
v2ray_conf_tw="${v2ray_conf_dir}/config_tw.json"
v2ray_conf_jp="${v2ray_conf_dir}/config_jp.json"
v2ray_conf_us="${v2ray_conf_dir}/config_us.json"
...
v2ray_qr_config_file_hk="/usr/local/vmess_hk.json"
v2ray_qr_config_file_tw="/usr/local/vmess_tw.json"
v2ray_qr_config_file_jp="/usr/local/vmess_jp.json"
v2ray_qr_config_file_us="/usr/local/vmess_us.json"
...
cp /usr/local/vmess_qr.json /usr/local/vmess_hk.json
cp /usr/local/vmess_qr.json /usr/local/vmess_tw.json
cp /usr/local/vmess_qr.json /usr/local/vmess_jp.json
cp /usr/local/vmess_qr.json /usr/local/vmess_us.json
...
docker_install() {
    ...
    if [[ "${ID}" == "centos" ]]; then
        ...
        snap install docker  && snap refresh
        docker pull teddysun/v2ray
        sleep 1
    fi
...
modify_path() {
    ...
    sed -i "/\"path\"/c \\\t  \"path\":\"${camouflage_hk}\"" ${v2ray_conf_hk}
    sed -i "/\"path\"/c \\\t  \"path\":\"${camouflage_tw}\"" ${v2ray_conf_tw}
    sed -i "/\"path\"/c \\\t  \"path\":\"${camouflage_jp}\"" ${v2ray_conf_jp}
    sed -i "/\"path\"/c \\\t  \"path\":\"${camouflage_us}\"" ${v2ray_conf_us}
    judge "V2ray 伪装路径 修改"
}
modify_alterid() {
    if [[ "on" == "$old_config_status" ]]; then
    ...
    sed -i "/\"alterId\"/c \\\t  \"alterId\":${alterID}" ${v2ray_conf_hk}
    sed -i "/\"alterId\"/c \\\t  \"alterId\":${alterID}" ${v2ray_conf_tw}
    sed -i "/\"alterId\"/c \\\t  \"alterId\":${alterID}" ${v2ray_conf_jp}
    sed -i "/\"alterId\"/c \\\t  \"alterId\":${alterID}" ${v2ray_conf_us}
...
modify_inbound_port() {
    ...
    if [[ "$shell_mode" != "h2" ]]; then
        PORT=$((RANDOM + 10000))
        sed -i "/\"port\"/c  \    \"port\":10000," ${v2ray_conf}
        sed -i "/\"port\"/c  \    \"port\":10001," ${v2ray_conf_hk}
        sed -i "/\"port\"/c  \    \"port\":10002," ${v2ray_conf_tw}
        sed -i "/\"port\"/c  \    \"port\":10003," ${v2ray_conf_jp}
        sed -i "/\"port\"/c  \    \"port\":10004," ${v2ray_conf_us}
    else
        sed -i "/\"port\"/c  \    \"port\":$10000," ${v2ray_conf}
        sed -i "/\"port\"/c  \    \"port\":$10001," ${v2ray_conf_hk}
        sed -i "/\"port\"/c  \    \"port\":$10002," ${v2ray_conf_tw}
...
modify_UUID() {
    ...
    sed -i "/\"id\"/c \\\t  \"id\":\"${UUID}\"," ${v2ray_conf_hk}
    sed -i "/\"id\"/c \\\t  \"id\":\"${UUID}\"," ${v2ray_conf_tw}
    sed -i "/\"id\"/c \\\t  \"id\":\"${UUID}\"," ${v2ray_conf_jp}
    sed -i "/\"id\"/c \\\t  \"id\":\"${UUID}\"," ${v2ray_conf_us}
    judge "V2ray UUID 修改"
...
domain_check() {
    read -rp "请输入你的域名信息(eg:www.steamsv.com):" domain
...
v2ray_conf_add_tls() {
    ...
    wget --no-check-certificate https://raw.githubusercontent.com/wulabing/V2Ray_ws-tls_bash_onekey/${github_branch}/tls/config.json -O config_hk.json
    wget --no-check-certificate https://raw.githubusercontent.com/wulabing/V2Ray_ws-tls_bash_onekey/${github_branch}/tls/config.json -O config_tw.json
    wget --no-check-certificate https://raw.githubusercontent.com/wulabing/V2Ray_ws-tls_bash_onekey/${github_branch}/tls/config.json -O config_jp.json
    wget --no-check-certificate https://raw.githubusercontent.com/wulabing/V2Ray_ws-tls_bash_onekey/${github_branch}/tls/config.json -O config_us.json
    modify_path
    modify_alterid
    modify_inbound_port
    modify_UUID
...
start_process_systemd() {
    ...
    docker run -d --network host --name default --restart=always -v /etc/v2ray:/etc/v2ray teddysun/v2ray /usr/bin/v2ray -config /etc/v2ray/config.json
    docker run -d --network host --dns-search=hk1.dnsunlock.com --name hk --restart=always -v /etc/v2ray:/etc/v2ray teddysun/v2ray /usr/bin/v2ray -config /etc/v2ray/config_hk.json
    docker run -d --network host --dns-search=tw1.dnsunlock.com --name tw --restart=always -v /etc/v2ray:/etc/v2ray teddysun/v2ray /usr/bin/v2ray -config /etc/v2ray/config_tw.json
    docker run -d --network host --dns-search=jp1.dnsunlock.com --name jp --restart=always -v /etc/v2ray:/etc/v2ray teddysun/v2ray /usr/bin/v2ray -config /etc/v2ray/config_jp.json
    docker run -d --network host --dns-search=us1.dnsunlock.com --name us --restart=always -v /etc/v2ray:/etc/v2ray teddysun/v2ray /usr/bin/v2ray -config /etc/v2ray/config_us.json
    judge "docker 启动"
}

enable_process_systemd() {
    systemctl enable docker
    judge "设置 docker 开机自启"
    if [[ "$shell_mode" != "h2" ]]; then
...
vmess_qr_config_tls_ws_hk() {
    cat >$v2ray_qr_config_file_hk <<-EOF
{
  "v": "2",
  "ps": "${domain}",
  "add": "${domain}",
...
vmess_qr_config_tls_ws_tw() {
    cat >$v2ray_qr_config_file_tw <<-EOF
{
  "v": "2",
  "ps": "${domain}",
  "add": "${domain}",
...
vmess_qr_config_tls_ws_jp() {
    cat >$v2ray_qr_config_file_jp <<-EOF
{
  "v": "2",
  "ps": "${domain}",
  "add": "${domain}",
...
vmess_qr_config_tls_ws_us() {
    cat >$v2ray_qr_config_file_us <<-EOF
{
  "v": "2",
  "ps": "${domain}",
  "add": "${domain}",
...
vmess_qr_link_image_hk() {
    vmess_link_hk="vmess://$(base64 -w 0 $v2ray_qr_config_file_hk)"
    {
        echo -e "${Green} HK导入链接:${vmess_link_hk} ${Font}"
        echo -e ""
    } >>"${v2ray_info_file}"
}

vmess_qr_link_image_tw() {
    vmess_link_tw="vmess://$(base64 -w 0 $v2ray_qr_config_file_tw)"
    {
        echo -e "${Green} TW导入链接:${vmess_link_tw} ${Font}"
        echo -e ""
    } >>"${v2ray_info_file}"
}

vmess_qr_link_image_jp() {
    vmess_link_jp="vmess://$(base64 -w 0 $v2ray_qr_config_file_jp)"
    {
        echo -e "${Green} JP导入链接:${vmess_link_jp} ${Font}"
        echo -e ""
    } >>"${v2ray_info_file}"
}

vmess_qr_link_image_us() {
    vmess_link_us="vmess://$(base64 -w 0 $v2ray_qr_config_file_us)"
    {
        echo -e "${Green} US导入链接:${vmess_link_us} ${Font}"
        echo -e ""
    } >>"${v2ray_info_file}"
}
...
vmess_link_image_choice() {
        ...
        echo "2: quantumult(无DNS解锁,请自行配置)"
...
menu() {
    ...
    echo -e "---authored from wulabing by steamsv--- \n"
    ...
    echo -e "—————————————— 查看信息 ——————————————"
    echo -e "${Green}3.${Font}  查看 实时访问日志"
    echo -e "${Green}4.${Font}  查看 实时错误日志"
    echo -e "${Green}5.${Font} 查看 V2Ray 配置信息"
    echo -e "—————————————— 其他选项 ——————————————"
    echo -e "${Green}6.${Font} 证书 有效期更新"
    echo -e "${Green}7.${Font} 卸载 V2Ray"
    echo -e "${Green}8.${Font} 更新 证书crontab计划任务"
    echo -e "${Green}9.${Font} 清空 证书遗留文件"
